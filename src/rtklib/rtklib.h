...
typedef struct {        /* satellite status type */
    ...
    uint8_t slip[NFREQ]; /* cycle-slip flag 同时记录了基站、流动站的周跳信息 */
...
// add by Qijia Zhao
// Restore the Single Difference ambiguity information
typedef struct sdambi_info {
	int epoch_s = -1, epoch_e = -1;
	int f = -1;
	int sat = -1;
	int num = 0;
	double sdbias = 0.0;
    double float_sdbias = 0.0;
	double var = 0.0;
	int sdflg = 0;
	bool Ismarg = false;
	int flag = 1; //ambiguity fix flag 1:float, 2:ready to fix, 3:fix
	double weight = 0.0;
	std::deque<double> bias_value;
	std::deque<double> bias_var;
	std::deque<int> nepochs;
} sdambi_info;

// Restore the Double Difference information
typedef struct ddinfo {
	int refsat = 0, ref_sdambi_index = -1;
	int urefsat = 0, uref_sdambi_index = -1;
	int f = -1, code = -1;
	int ref_index = -1, uref_index = -1;
	bool isValid = true;
	bool isPrior = true;
	double ddbias = 0.0;
	double var = 0.0;
...
typedef struct node_info {
	...
	bool Dopp_Valid[MAXSAT][NFREQ];
	gtime_t time;
	uint8_t stat = SOLQ_FLOAT;
	obsd_t obs[MAXOBS * 2];
	double *xf;
	double *xa;
	double *Pf;
	...
	double ratio = 0.0;
	int nfix = 0;
	int nn = 0, nu = 0, nr = 0, nv = 0, ns = 0, nd = 0, nx = 0, na = 0, nb = 0, nsd = 0, ndopp = 0;
	double tt;
	int ir[MAXSAT] = { 0 }, iu[MAXSAT] = { 0 }, nsat[MAXSAT] = { 0 };
	ddinfo *ddmeas, *ddfix;
	double *Rsd = nullptr;
	double *zdbase = nullptr;

	double clockbias = 0.0;
	double clockshift = 0.0;
	bool USE_Dopp = true;
	bool USE_DD = true;
...
typedef struct {        /* RTK control/result type */
	...
	// add by Qijia Zhao
    // Restore information during the preprocessing of RTKLIB
	int nn, nu, nr, nv, ns, nd, nb;
	int ir[MAXSAT], iu[MAXSAT], nsat[MAXSAT];
	ddinfo ddmeas[MAXOBS * NFREQ * 2 + 1];
	ddinfo ddfix[MAXOBS * NFREQ * 2 + 1];
	double *xfix;
	double dt; 
    double bl;
	// end
...
EXPORT int pntpos(const obsd_t *obs, const int n, const nav_t *nav,
