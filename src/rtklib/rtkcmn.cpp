                  ...
                  const double *R, int n, int m)
{
    ...
    ix=imat(n,1); for (i=k=0;i<n;i++) if (x[i]!=0.0&&P[i+i*n]>0.0) ix[k++]=i; // k是非零状态的个数，ix存储非零状态的索引
...
static int mkdir_r(const char *dir)
{
    ...
    if ((p=strrchr(pdir,FILEPATHSEP))) {
        *p='\0';
        h=FindFirstFile((LPCTSTR)pdir,&data);
        if (h==INVALID_HANDLE_VALUE) {
            if (!mkdir_r(pdir)) return 0;
        }
        else FindClose(h);
    }
    if (CreateDirectory((LPCTSTR)dir,NULL)||GetLastError()==ERROR_ALREADY_EXISTS) {
        return 1;
    }
#else
    FILE *fp;
